...
| SNAP-20260911-022 | code | 完成 API-ready 前端集成验收；修复 390px 问茶嵌入冲泡布局溢出，保存 mock 桌面、mock 移动和 live 后端不可达三类浏览器证据 | `npm test`（13 项通过）；`npm run build`；Edge Chromium 验证 mock 1440×900、390×844、移动后台菜单/容器滚动及 live 503 重试，控制台 0 error/0 warning | 随本次提交推送至 GitHub `main` |
| SNAP-20260911-023 | code | 顶部与移动导航改为首页、问茶、茶叶品类、工作台四个子页面入口；新增工作台登录页、mock/live A01--A04 认证映射、后台路由守卫与退出流程 | `npm test -- --run`（15 项通过）；`npm run build`；Edge Chromium 验证错误/正确登录、退出、未登录重定向、1440 桌面与 390×844 移动布局，控制台 0 error/0 warning | 随本次提交推送至 GitHub `main` |
