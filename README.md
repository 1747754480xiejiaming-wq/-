...
- Mock 契约层：演示种子已通过 mock API 状态机提供；已覆盖公开可见性、来源撤回、商品下架/重新上架、版本 ETag 与数据运营/审核人权限边界，页面接入将在后续批次完成。
- 运行模式：已实现 `VITE_API_MODE=mock|live` 服务工厂、公开/后台 live 端点映射和 React 根服务容器；后台命令透传调用方幂等键、CSRF 与 ETag，401 会使内存中的 CSRF 失效。页面迁移将在下一批完成。
