...
- 顶部入口：桌面主导航与移动底栏统一为“首页 / 问茶 / 茶叶品类 / 工作台”；“问茶”进入独立问答页，“茶叶品类”进入商品总览，“工作台”进入后台登录页。
- 管理后台：按数据运营、客户审核、线索跟进和项目管理员拆分四套职能前端；页面已改为读取 A03 权限、C01--C13 内容服务与 M01/M03/M07 线索服务。内容与导入合并，审核通过自动发布，数据运营和项目管理员可直接上下架商品。
- 后台登录：已完成前端登录、路由守卫和退出流程；mock 使用 `demo / demo123456`，live 按 A01 获取登录前 CSRF、A02 建立会话、A03 校验会话、A04 退出。真实账号和 Cookie 会话仍等待后端服务提供。
- 响应式：本轮复核 1440×900 桌面、390×844 问茶与后台；移动问茶 `scrollWidth=390`，后台宽表格只在容器内滚动。
...
- 数据状态：公开目录、批次详情、问茶冲泡和咨询已全部经统一服务层读取；默认使用契约化 mock 数据，可切换至 `/api/v1` live 后端。尚未接入正式后端、数据库、身份系统或真实大模型。
- 联调基础：已增加可切换的 API 运行时配置、统一成功/失败信封解析、cookie 请求、CSRF/ETag/幂等请求头和 API 错误归一化；后续批次将把现有演示状态迁移至 mock/live 适配器。
- Mock 契约层：演示种子已通过 mock API 状态机提供；已覆盖公开可见性、来源撤回、商品下架/重新上架、版本 ETag 与数据运营/审核人权限边界。
- 运行模式：已实现 `VITE_API_MODE=mock|live` 服务工厂、公开/后台 live 端点映射和 React 根服务容器；公开端使用严格 snake_case 传输 DTO，目录、详情、U11→U07 问茶冲泡和 U12 咨询均已迁移。后台四平台通过 hooks 调用服务，写命令透传幂等键、CSRF 与 ETag，401 会使内存中的 CSRF 失效；live 导入在文件服务未接入前明确禁用，不伪造成功。
...
- 工作台登录：<http://127.0.0.1:4173/#/login>
...
默认运行契约化 mock。复制 `prototype/.env.example` 为 `.env.local` 后可切换：
...
`live` 模式只访问配置的 `/api/v1` 服务，不回退到演示种子；后端不可达时页面会显示可重试错误。
...
5. 从“工作台”进入登录页，使用 `demo / demo123456` 登录，再依次切换数据运营、客户审核人、线索跟进和项目管理员身份。
