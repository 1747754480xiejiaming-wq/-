...
- 运行模式：已实现 `VITE_API_MODE=mock|live` 服务工厂、公开/后台 live 端点映射和 React 根服务容器；公开端使用严格 snake_case 传输 DTO，目录、详情、U11→U07 问茶冲泡和 U12 咨询均已迁移。后台命令透传调用方幂等键、CSRF 与 ETag，401 会使内存中的 CSRF 失效。
