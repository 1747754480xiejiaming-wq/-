...
| O | 数据运营 | 内容草稿读写、文件上传、提交审核；具备导入权限时导入 |
| R | 客户审核人 | 按审核领域查看资料、审批、发布、下线；不能审核自己修改的版本 |
| L | 线索跟进人员 | 查询、跟进及经授权导出咨询；不因此获得内容发布权限 |
| A | 项目管理员 | 管理账号、权限、导入和审计；不自动获得专家审批权 |
...
- 咨询、反馈、埋点、问答、创建草稿、提交审核、审批、发布、下线、复制版本、导入提交和导出创建必须携带 `Idempotency-Key` UUID。
...
| C10 | POST `/admin/content/{resource}/{id}/review` | `{revision:integer,decision:approve或reject,comment:string}` | `ContentDetail<T>` | 对应领域 R + If-Match；禁止审核自己的修改；拒绝必须说明原因 |
| C11 | POST `/admin/content/{resource}/{id}/publish` | `{revision:integer,comment?:string}` | `ContentDetail<T>` | 对应领域 R + If-Match；仅 approved；重新核验有效期、资料权利和依赖 |
| C12 | POST `/admin/content/{resource}/{id}/withdraw` | `{revision:integer,reason:string}` | `ContentDetail<T>` | 对应领域 R 或 A + If-Match；立即阻止公开读取，并触发索引失效 |

不提供业务内容硬删除接口。历史发布版本不得被 PATCH 覆写；回滚内容必须复制旧版本到新草稿，重新审核发布。单个 stable ID 同时只允许一个活动发布版本、一个工作版本。新版本发布事务性替换活动指针，旧版本变为 superseded。
...
状态允许：`draft → pending_review → approved → published`；`pending_review → rejected → draft`；`published → withdrawn/expired/superseded`。到期由读取条件立即生效，后台任务补记 expired；不得依赖定时任务及时运行才阻止读取。approved 不能直接编辑，须复制新草稿。撤回授权同时阻断依赖资料，不覆盖已有审计事实。
...
`AuditEntry={id,actor_id?,action,resource,object_id?,before_revision?,after_revision?,summary_redacted,request_id,created_at}`。审计只追加；查看联系方式、导出、下载来源、导入、审核、发布、撤权、账号权限调整均记录；普通运营账号没有删除审计能力。
...
| 内容编辑及审核 | C01—C12、I06—I07 | 草稿、校验失败、审核拒绝、版本冲突、对比、待发布、下线 |
...
5. **完成后台内容循环。** 导入→校验→草稿→审核→发布→前台可见→撤权→前台和问答不可见；加入草稿与线上版本并存验证。
...
| T22 | 备份恢复和应用版本回滚 | 数据与附件引用可恢复；迁移兼容性经验证，不能只回滚前端文件 |
